...
系统 SHALL 为每次 LLM Tool 调用追加 `a_tool_call_log` 记录，并在助手消息保存时写入本轮 Tool 调用聚合信息。Tool 日志写入失败 MUST NOT 阻断用户问答主流程。

#### Scenario: 成功调用 Tool 记录成功日志
